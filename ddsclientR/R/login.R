...
login <- function(username=NA, password=NA, sessionToken=NA, sa_apiKey=NA, rememberMe=FALSE, endpoint=defaultEndpoint) {
  ## parameters must be of length 1
  if (sum(is.na(username),is.na(password))==0) {
    .scrapesessioncookie
  }


    & is.na(sessionToken)==TRUE || length(sa_apiKey) != 1))
...
.scrapesessioncookie <- function(username=NA,password=NA,endpoint=defaultEndpoint) {
  browseURL(endpoint)
